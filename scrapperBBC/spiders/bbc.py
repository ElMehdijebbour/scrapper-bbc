...
class BBCSpider(scrapy.Spider):
    ...
    def parse_sub_category(self, response):
        ...
        if not self.is_around_bbc_scraped:
            ...
            for body_article in body_articles_articles:
                        article_item["subtitle"]=body_article.css('.gs-c-promo-summary::text').get(),
                        article_item["title"]=body_article.css('.gs-c-promo-heading h3::text').get(),
                        article_item["url"]=response.urljoin(body_article.css('.gs-c-promo-heading ::attr(href)').get())

                        request=scrapy.Request(article_item["url"], callback=self.parse_article, dont_filter=True,meta=article_item,)
    ...
    def parse_article(self,response):
        ...
        #ggeting data in ISO8061 format
        if response.css('.ssrcss-1if1g9v-MetadataText time ::attr(datetime)').get() is not None:
